...
_nodes_4_light_root=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)

_submit_nodes_4_light_jobs() {
    local n dir job_id calculation
    local status=0

    for n in 1 2 3 4 6 7 8; do
        dir="${_nodes_4_light_root}/supercell-${n}"
...
_submit_nodes_4_light_jobs
_nodes_4_light_status=$?
unset -f _submit_nodes_4_light_jobs
unset _nodes_4_light_root

# Work both when sourced (recommended) and when executed directly.
if [ "$_nodes_4_light_status" -eq 0 ]; then
    unset _nodes_4_light_status
