#!/usr/bin/env python3
"""Extract and unwrap polarization along the BiFeO3 distortion path."""
...
def parse_output(
    ...
    """Return polarizations, projected quanta, and the direct cell."""
    cartesian = None
    ...
    directive_quanta = tuple(abs(quanta[index]) for index in (0, 2, 4))
    return (
        cartesian,
        tuple(directive_polarizations),
        directive_quanta,
        tuple(lattice_vectors[-3:]),
    )
...
def conventional_representation(
    ...
    cartesian = tuple(
        sum(reduced[direction] * quantum_vectors[direction][component]
    ...
    for functional_dir in functional_dirs:
        ...
        for geometry, path in sorted(files):
            ...
            if previous_folded_directives is None:
                # The centrosymmetric reference can be reported at either +Q/2
                # or -Q/2.  Put all three symmetry-equivalent directions on the
                # branch nearest P1 before following the path continuously.
                initial_reference = directive_polarizations[0]
                initial_branch_shifts = tuple(
                    nearest_integer((initial_reference - raw_value) / direction_quantum)
                    for raw_value, direction_quantum in zip(
                        directive_polarizations, directive_quanta
                    )
                )
                folded_directives = tuple(
                    ...
                    for raw_value, shift, direction_quantum in zip(
                        directive_polarizations,
                        initial_branch_shifts,
                        directive_quanta,
                    )
                )
                branch_shifts = initial_branch_shifts
            else:
                directive_branch_shifts = tuple(
                ...
                folded_directives = tuple(
                    ...
                    for raw_value, shift, direction_quantum in zip(
                        directive_polarizations,
                        directive_branch_shifts,
                        directive_quanta,
                    )
                )
                branch_shifts = directive_branch_shifts
            (
