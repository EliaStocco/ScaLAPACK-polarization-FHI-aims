...
pivot["diff"] = pivot["pol"] - pivot["scf"]
...
for method in pivot["method"].unique():
    sub = pivot[pivot["method"] == method].sort_values("cores")
...
ab = AnnotationBbox(imagebox, (600, 800), frameon=False)
ax.add_artist(ab)
...
# ---- PRIMARY x ticks ----
ax.xaxis.set_major_locator(mticker.FixedLocator([128, 256, 512, 1024]))
